...
public class ChlorophyteBullet extends Bullet implements Actionable {
    ...
    public void action(World world) {
        if (time < 60) {
            ...
            if (distance < speed2) {
                PosTool.movePos(position, PosTool.getDirection(position, target2), PosTool.getSlope(position, target2), (float) distance);
            } else {
                PosTool.movePos(position, PosTool.getDirection(position, target2), PosTool.getSlope(position, target2), speed2);
            }
